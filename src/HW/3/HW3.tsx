...
export const HW3 = () => {
  ...
  const handleChange = (e:ChangeEvent<HTMLInputElement>) => {
    setCurrentText(e.target.value); // здесь надо обновлять текущее значение из инпута
  };

  const handleSave = () => {
    // ЗАСЕТАТЬ БЫ ТЕКСТ В texts И НЕ ПОТЕРЯТЬ НАПУТСТВИЕ ИЗ ПРОШЛОГО ВЕКА)
    // А ЗАТЕМ УБРАТЬ ЗА СОБОЙ В currentText
    setTexts([...texts, currentText]); // добавляем новый текст в массив
    setCurrentText(''); // очищаем input после сохранения
  };
